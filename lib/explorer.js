...
module.exports = {
  ...
  get_masternodecount: function(cb) {
    ...
    request({uri: uri, json: true}, function (error, response, body) {
      if (settings.baseType === 'pivx')
        return cb(body.total, body.stable);
      else
        return cb(body);
    });
  },
  ...
  calculate_total: function(vout, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      return cb(total, Tfee);
    });
  },

  prepare_vout: function(vout, txid, vin, fee, cb) {
    var arr_vout = [];
    var arr_vin = [];
    arr_vin = vin;
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type !== 'nonstandard' && vout[i].scriptPubKey.type !== 'nulldata' && vout[i].scriptPubKey.hasOwnProperty("addresses")) {
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses[0], function(unique, index) {
          ...
          if (vout[i].scriptPubKey.type === 'nulldata') {
              arr_vout.push({addresses: Buffer.from( vout[i].scriptPubKey.asm.slice(10), 'hex').toString(), amount: 0, fee: 1});
          }
      ...
      if (vout[0].scriptPubKey.type === 'nonstandard') {
        if ( arr_vin.length > 0 && arr_vout.length > 0 ) {
          if (arr_vin[0].addresses === arr_vout[0].addresses) {
            ...
            arr_vin.shift();
            return cb(arr_vout, arr_vin, fee);
          } else {
              return cb(arr_vout, arr_vin, fee);
          }
        } else {
