  ...
  lib.get_rawtransaction(txid, function(tx){
    if (tx != 'There was an error. Check your console.') {
      lib.get_block(tx.blockhash, function(block){
        if (block) {
          lib.prepare_vin(tx, function(vin, fee) {
            lib.prepare_vout(tx.vout, txid, vin, fee, function(vout, nvin, fee) {
              lib.syncLoop(nvin.length, function (loop) {
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].amount, type, function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
                  ...
                  lib.calculate_total(vout, function(total, Tfee){
                    if (Tfee)
                      var newTx = new Tx({
                        txid: tx.txid,
                        vin: nvin,
...
module.exports = {
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({'total': {$gt: min}, 'vout.0': {$exists: true}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  update_coinmarketcap_db: function(coin, cb) {
    coinmarketcap.get_data(coin, function(err, obj) {
      ...
      Cmc.update({ symbol: coin }, {
        ...
        volume_24h_usd: obj.quote.USD.volume_24h,
        market_cap_usd: obj.quote.USD.market_cap,
        available_supply: obj.total_supply,
