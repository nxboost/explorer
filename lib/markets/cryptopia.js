...
function get_summary(coin, exchange, cryptopia_id, cb) {
    ...
    request({ uri: base_url + '/ticker/', json: true }, function (error, response, body) {
        if (error) {
            ...
            summary['bid'] = body.Data[1]['bid'].toFixed(8);
            summary['ask'] = body.Data[1]['ask'].toFixed(8);
            summary['volume'] = body.Data[1]['24H_volume'];
            summary['high'] = body.Data[1]['high'].toFixed(8);
            summary['low'] = body.Data[1]['low'].toFixed(8);
            summary['last'] = body.Data[1]['last_price'].toFixed(8);
            summary['change'] = body.Data[1]['change'];
